...
public class ReviewController {
	...
	public void getReview(@RequestParam(required = true)Integer current,@RequestParam(required = true)Integer pageCount,@RequestParam(required = true)Integer classId,HttpServletResponse response) throws IOException{
		PageRequest request = PageRequest.build(current, null);
		request.setPageCount(pageCount);
		int total = reviewService.countAll();
		List<Review> list = reviewService.getPage(request,classId);
